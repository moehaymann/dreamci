...
router.post("/cinemaAdd",checkAdmin,cpUpload, async function(req,res){
    ...
    if(req.files.cover.length>0) cinema.cover = "/images/uploads/" + req.files.cover[0].filename;
...
router.get("/cinemaDetail/:id", checkAdmin, async function(req,res){
    const cinema = await Cinemas.findById(req.params.id).populate("createdBy","email").populate("updatedBy","email");
...
router.post("/cinemaUpdate",checkAdmin, cpUpload, async function(req,res){
    ...
    if(req.files.cover && req.files.cover.length>0){update.cover = "/images/uploads/" + req.files.cover[0].filename;
...
router.get("/cinemaDetete/:id", checkAdmin, async function(req,res){
    const cinema = await Cinemas.findById(req.params.id);
    if(cinema){
        try{
            cinema.gallery.forEach(function(filePath){
...
router.get("/movieDetail/:id",checkAdmin,async function(req,res){
    const movie = await Movies.findById(req.params.id).populate("cinemaId","name").populate("createdBy","email").populate("updatedBy","email");
...
router.post("/checkCinemaSeat",checkAdmin,async function(req,res){
    try{
        const seats = await Seats.find({cinemaId: req.body.id}).sort({screenNo:1,row:1,no:1});
...
router.post("/confirmBooking", checkAdmin, async function(req,res){
    ...
    try{
        const data = await Bookings.findByIdAndUpdate(req.body.id,{$set: update});
        res.json({status: true});
    }catch(e){
        res.json({status: false});
    }
